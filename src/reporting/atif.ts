...
export function toAtifTrajectory(
  ...
  return {
    ...
    agent: {
      ...
      tool_definitions: [playwrightRunToolDefinition()],
      extra: {
    ...
    notes: 'Generated by Langwright from observable Playwright tool execution. Hidden model reasoning is intentionally not recorded.',
    final_metrics: {
...
function buildUserStep(stepId: number, block: AgentInstructionBlock, fallbackTimestamp: string): AtifStep {
  const label = block.kind === 'expectation' ? 'Expectation' : 'Step';

  return {
    step_id: stepId,
    timestamp: fallbackTimestamp,
    source: 'user',
    message: `${label} ${block.id}:\n${block.text}`,
    extra: {
      block_id: block.id,
      kind: block.kind,
    },
...
function buildAgentStep(
  stepId: number,
  block: AgentInstructionBlock | undefined,
  events: TurnEvent[],
  ...
  return {
    ...
    tool_calls: events.map(({ event, eventIndex }) => ({
      ...
      function_name: 'playwright_run',
      arguments: {
        body: event.code,
        purpose: event.purpose,
        blockIds: event.blockIds,
        contributesToNativeCode: event.contributesToNativeCode,
      },
      extra: {
...
function agentStepMessage(block: AgentInstructionBlock | undefined, callCount: number): string {
  if (!block) {
    return callCount > 0
...
function toolCallId(index: number): string {
  return `call-playwright-run-${index + 1}`;
}

function playwrightRunToolDefinition(): AtifToolDefinition {
  return {
    type: 'function',
    function: {
      name: 'playwright_run',
      description: 'Run Playwright Test code against the current test page.',
      parameters: {
        type: 'object',
        properties: {
          body: {
            type: 'string',
            description: 'Playwright Test code to run inside the current test async function.',
          },
          purpose: {
            type: 'string',
            enum: ['probe', 'action', 'assertion', 'final'],
            description: 'Why this tool call was made.',
          },
          blockIds: {
            type: 'array',
            items: { type: 'string' },
            description: 'Ordered Langwright instruction block IDs implemented by this code.',
          },
          contributesToNativeCode: {
            type: 'boolean',
            description: 'Whether this body should be considered for native Playwright output.',
          },
        },
        required: ['body'],
      },
