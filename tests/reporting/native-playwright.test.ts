...
void describe('buildNativePlaywrightArtifact', () => {
  void it('emits complete native code 1:1 from passing scenarios', () => {
  ...
  void it('labels action and assertion spans by kind', () => {
    const artifact = buildNativePlaywrightArtifact(
  ...
  void it('excludes a failed scenario and marks the artifact partial', () => {
    const artifact = buildNativePlaywrightArtifact(
      [
  ...
  void it('returns invalid when no green scenario produced code', () => {
  ...
  void it('strips return/console/notes diagnostic lines from generated code', () => {
    const artifact = buildNativePlaywrightArtifact(
      [
        record('block-1', {
